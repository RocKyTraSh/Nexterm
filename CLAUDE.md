...
10. **Следующее**: (а) remote (`-R`) через jump-chain (нужен `forwarded-tcpip` поверх target-хопа цепочки); (б) отдельный `CredentialRef` для key-passphrase (сейчас stored secret трактуется как пароль); (в) несколько `-R` на одно соединение (роутинг входящих по `connected_port`); (г) agent forwarding для exec (non-shell) если понадобится.
11. `SqliteProfileStore` за трейтом `ProfileStore` + миграции.
12. Qt-скелет: одно окно + одна терминальная вкладка поверх `AppCore`; sidebar — модель поверх `ProfileStore`.
