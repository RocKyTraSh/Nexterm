...
- `crates/core/src/model/profile.rs` — доменная модель, `CredentialRef`, `ProtocolSettings` (tagged enum, расширяется новыми вариантами). **Раздельные credential-refs**: `ConnectionProfile.credential` = password; `SshSettings.key_passphrase` (`Option<CredentialRef>`, `#[serde(default)]`) = passphrase зашифрованного ключа.
- `crates/credentials/src/{secret,backend,memory,keyring_os}.rs` — безопасность секретов. `ResolvedCredentials` (в `protocols/traits.rs`) держит независимые `password` + `key_passphrase: Option<Secret>` с редактирующим `Debug`.
...
**Готово (с тестами по ключевым крейтам):** workspace и граф зависимостей; модели/конфиг/события/реестр сессий; `ProfileStore` (JSON); `Secret` + `CredentialStore` (memory + `keyring-os`); трейты протоколов + SSH mock; **реальный SSH/SFTP через `russh` (фича `ssh-russh`): PTY-shell `RusshSession`, `RusshSftp`, auth agent→key→password→keyboard-interactive, known_hosts-политика — pure-логика под юнит-тестами, end-to-end проверено против localhost sshd**; **переиспользуемый примитив `SshConnection`**; **multi-hop jump-host chains через вложенный `direct-tcpip` для shell И SFTP** (`SshConnection::connect_via_jump_chain`, `Connector::connect_*_via_jump_chain`, `MAX_JUMP_CHAIN=8`, CLI `ssh-chain-connect`/`sftp-chain-ls` + single-hop `ssh-jump-connect`/`sftp-jump-ls`); **`Connector` trait с методами jump/chain/SFTP**; **оркестрация jump-chains в `AppCore`** (`connect`/`connect_sftp` разворачивают цепочку gateway-профилей из `ProfileStore` + резолвят секреты, cycle/depth-проверки, pure-тесты в default-сборке); **tunnel driver `RusshTunnelDriver`** (фича `ssh-russh`) — **все три вида форвардинга**: **local `-L`** (CLI `tunnel-local`), **dynamic SOCKS5 `-D`** (CLI `tunnel-socks`, парсер `socks5.rs` в default-сборке) и **remote `-R`** (CLI `tunnel-remote` direct + `tunnel-remote-chain` через jump-chain — `tcpip-forward` на FINAL TARGET); **SSH agent forwarding для shell** (`SshSettings.agent_forwarding`, флаг `--agent-forwarding`, default off; direct и через jump-chain — только target shell); **раздельные credential-refs для password vs key-passphrase** (`SshSettings.key_passphrase`, `AppCore::resolve_credentials` тянет оба секрета в независимые поля, не смешивает; CLI `--key-passphrase-env`); подсветка/alt-screen/PTY(feature); local-shell транспорт через `AppCore` (`LocalShellConnector`/`LocalPtySession`, фича `local-pty`, тест с реальным PTY); менеджер туннелей + mock + тесты; HTTP + scheduler мини-серверы; `ui-common` (app/safety/multiexec/macros/conflict); CLI (+ команды `local-shell`, `ssh-connect`, `ssh-jump-connect`, `ssh-chain-connect`, `tunnel-local`, `tunnel-socks`, `tunnel-remote`, `tunnel-remote-chain`, `sftp-ls`, `sftp-jump-ls`, `sftp-chain-ls`); qt/gtk заготовки; xtask; README.

**НЕ сделано:** несколько `-R` на одно SSH-соединение; SQLite-хранилище; любой GUI; вендорные пресеты подсветки; полная SGR-aware подсветка; прочие мини-серверы (TFTP/FTP/SSH/Telnet/NFS/VNC); RDP/VNC-клиенты; финальный credential-UX (CLI берёт password/passphrase из env — dev-only; GUI будет писать в keyring через `CredentialStore`); key-passphrase ref в `profiles add-ssh`.

**ВАЖНО:** сборка **верифицирована** (`cargo build`/`cargo test --workspace` + `--features ssh-russh` зелёные, 2026-06-16, rustc 1.96; `fmt --check` и `clippy --all-targets` — 0 варнингов на default и на `ssh-russh` для protocols/tunnels/cli/ui-common). Дефолтная сборка чистая (`cargo tree --no-default-features` не содержит russh; default-`tunnels` не тянет `rrs-protocols`, но **содержит SOCKS5-парсер `socks5.rs`**; **`ui-common` не имеет фичи `ssh-russh` — jump-chain-оркестрация трейтовая и собирается/тестируется в default**). `local-pty`-тест прогоняет реальный `/bin/sh`; `ssh-russh` — **8 pure-тестов protocols (вкл. `plan_agent_forwarding`, `ResolvedCredentials`-Debug-редакция)**; **19 pure-тестов tunnels**; **2 pure-теста cli** (`parse_hop`/`jump_password_env`, фича `ssh-russh`); **25 pure-тестов ui-common (вкл. credential-резолв password/passphrase, default-сборка)**; **6 pure-тестов core** (agent_forwarding + key_passphrase serde/default/back-compat); плюс ignored live: `sftp_roundtrip`, `jump_host_roundtrip`, `sftp_jump_roundtrip`, `jump_chain_roundtrip`, `sftp_jump_chain_roundtrip`, `agent_forwarding_roundtrip`, `encrypted_key_passphrase_roundtrip` (protocols), `local_tunnel_roundtrip` + `dynamic_socks_roundtrip` + `remote_tunnel_roundtrip` + `remote_tunnel_chain_roundtrip` (tunnels) — все требуют sshd. Первый шаг любой сессии всё равно — `cargo build` + `cargo test --workspace`.
...
11. ~~отдельный `CredentialRef` для key-passphrase~~ — **готово**: `SshSettings.key_passphrase` (`Option<CredentialRef>`, `#[serde(default)]`), `AppCore::resolve_credentials` резолвит password+passphrase в независимые поля (dangling ref → ошибка), `try_key_auth` использует только passphrase, CLI `--key-passphrase-env` (ssh-connect/sftp-ls/ssh-chain-connect), `AppCore::set_profile_key_passphrase`.
12. **Следующее**: (а) несколько `-R` на одно соединение (роутинг входящих по `connected_port`); (б) key-passphrase ref в `profiles add-ssh` (keyring); (в) agent forwarding для exec (non-shell) если понадобится.
13. `SqliteProfileStore` за трейтом `ProfileStore` + миграции.
14. Qt-скелет: одно окно + одна терминальная вкладка поверх `AppCore`; sidebar — модель поверх `ProfileStore`.
...
- **Раздельные credentials (password vs key-passphrase)**: модель хранит ДВА независимых `CredentialRef` — `ConnectionProfile.credential` (password) и `SshSettings.key_passphrase` (passphrase). `AppCore::resolve_credentials` резолвит каждый в свой field `ResolvedCredentials` (`password`/`key_passphrase`) и НЕ смешивает; `resolve_ref` возвращает понятную ошибку для dangling-ref (есть ссылка, нет секрета в сторе). В russh: `try_password_auth`/`try_keyboard_auth` читают ТОЛЬКО `password`; `try_key_auth` (`load_secret_key`) — ТОЛЬКО `key_passphrase`; agent — без секрета. Encrypted key без passphrase → `load_secret_key` ошибается, warn с контекстом, fall-through к следующему методу (auth-order сохранён). Passphrase НЕ хранится в профиле (только ref); jump-chain резолвит passphrase per-hop через `resolve_jump_chain`→`resolve_credentials`. CLI dev-harness: `--key-passphrase-env` (env, не печатается); `profiles add-ssh` пока без passphrase-ref (TODO: keyring через GUI).
...
## Итог итерации (2026-06-16): отдельный CredentialRef для key-passphrase

- **Наблюдение**: protocol-слой УЖЕ разделял секреты — `ResolvedCredentials { password, key_passphrase }` существовал, `try_key_auth` читал только `key_passphrase`, `try_password_auth`/`try_keyboard_auth` — только `password`. Не хватало **модели** (где взять ref на passphrase) и **резолва** (`AppCore` тянул только password). Поэтому фокус — модель + резолв + CLI, без переписывания auth.
- **Модель**: `SshSettings.key_passphrase: Option<CredentialRef>` (`#[serde(default)]`, default `None`, back-compat). Password-ref остался на уровне профиля (`ConnectionProfile.credential`) с doc-TODO про более точную раскладку. Серде/back-compat/coexistence — 3 новых core-теста.
- **AppCore**: `resolve_credentials` теперь резолвит password (профиль) И passphrase (`ssh.key_passphrase`) в независимые поля; `resolve_ref` — единый helper, dangling-ref → понятная ошибка (для обоих). `set_profile_key_passphrase` — сохранить passphrase под её ref. jump-chain автоматически тянет passphrase per-hop (через тот же `resolve_credentials`). 7 новых ui-common-тестов (password-only/passphrase-only/both/missing-error×2/old-profile/chain-per-hop).
- **russh**: `try_key_auth` уточнил warn (encrypted key без passphrase → подсказка), доком зафиксировал «копия passphrase в String рвёт zeroize, как password». `ResolvedCredentials`-Debug редактирует оба секрета (новый pure-тест в `traits.rs`, default-сборка).
- **CLI**: `--key-passphrase-env` у `ssh-connect`/`sftp-ls`/`ssh-chain-connect` (chain — общая passphrase на все хопы, dev-only). `ssh-connect` ставит отдельный `key_passphrase`-ref и стораджит через `set_profile_key_passphrase`; `sftp-ls`/chain используют `creds_from_envs`. Passphrase только из env, не печатается. Live `#[ignore]` `encrypted_key_passphrase_roundtrip` (env `NEXTERM_SSH_TEST_ENCRYPTED_KEY` + `NEXTERM_SSH_TEST_KEY_PASSPHRASE`).
- Проверки зелёные: `fmt --check`; `build`/`test`/`clippy --all-targets` (default + `ssh-russh` для protocols/tunnels/cli; ui-common/core/credentials default) — 0 варнингов. `cargo tree --no-default-features` без russh. Модель-изменения — в default-сборке (без новых зависимостей).
- **Следующий шаг**: несколько `-R` на одно соединение; key-passphrase ref в `profiles add-ssh`; затем Qt-скелет поверх `AppCore`.
