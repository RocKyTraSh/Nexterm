...
7. **Следующее**: (а) проброс `agent_forwarding` (seam в модели есть); (б) remote (`-R`) / dynamic SOCKS (`-D`) туннели на том же `direct-tcpip`-примитиве; (в) отдельный `CredentialRef` для key-passphrase (сейчас stored secret трактуется как пароль).
8. `SqliteProfileStore` за трейтом `ProfileStore` + миграции.
9. Qt-скелет: одно окно + одна терминальная вкладка поверх `AppCore`; sidebar — модель поверх `ProfileStore`.
